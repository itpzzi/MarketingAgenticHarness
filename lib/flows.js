...
function trace(ctx, paradigm, label, detail) {
  const activityType = paradigm.includes('Sessão')
    ? 'tool'
    : paradigm.includes('Grafo') || paradigm.includes('Supercérebro') || paradigm.includes('App') || paradigm.includes('Dados') || paradigm.includes('Contexto') || paradigm.includes('criativos')
      ? 'resource'
      : 'reasoning';
  const event = { paradigm, label, detail, activityType, at: new Date().toISOString() };
...
function crossAdsAndLeads() {
  const ads = readResource('ads').ads;
  const rows = ads.map((ad) => {
    const leads = readResource('leads', { utm_content: ad.utm_content }).leads;
...
async function rlmSummarizeManual(ctx, { apiKey, model }) {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    partials.push(offline ? `[bloco ${index + 1} — resumo indisponível offline] ${chunk.slice(0, 160)}...` : text);
  }
  return partials;
}
...
async function* fullPipelineFlow(ctx, { message, apiKey, model }) {
  ...
  trace(ctx, 'Grafo de Estados (determinístico)', 'Consultando desempenho e leads', 'Dados de Meta Ads e CRM foram combinados para calcular os indicadores.');
...
async function* diagnosticoFlow(ctx, { message, apiKey, model }) {
  trace(ctx, 'Dados de campanha', 'Consultando anúncios e leads', 'Levantando indicadores por anúncio.');
  const rows = crossAdsAndLeads();

  trace(ctx, 'Contexto da conta', 'Consultando conversas recentes', 'Buscando sinais de desempenho e decisões pendentes.');
  const conv = readResource('conversations', { query: 'cpa' }).items;

  trace(ctx, 'Contexto da conta', 'Consultando linha do tempo', 'Buscando eventos recentes relevantes.');
  const timeline = readResource('timeline', { since: '2026-08-10T00:00:00-03:00' }).events;
...
async function* pautaFlow(ctx, { message, apiKey, model }) {
  trace(ctx, 'Contexto da conta', 'Consultando histórico recente', 'Reunindo eventos e conversas para a reunião.');
  const timeline = readResource('timeline', { since: '2026-08-14T00:00:00-03:00' }).events;
...
async function* creativeFlow(ctx, { message, apiKey, model }) {
  trace(ctx, 'Análise de criativos', 'Consultando ranking de criativos', 'Identificando a recomendação para cada peça.');
  const ranking = readResource('creative_ranking').ranking;
